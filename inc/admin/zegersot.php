...
$count=count($id)-1;
for($i=0;$i<$count;$i++)
$sql=mysql_query("DELETE FROM $zegersot_table_name WHERE id = ".$id[$i]." LIMIT 1");
if($sql)
...
$query	=	mysql_query("select * from $zegersot_table_name ORDER BY id DESC");
for ( $i = 0 ; $i < $limit ; $i++ )
	{
